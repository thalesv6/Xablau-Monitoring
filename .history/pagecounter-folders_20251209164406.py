...
def count_pages_by_folder_optimized(root_directory='G:\My Drive\XABLAU\7.VICTORIA'):
    """
    ...
    folders = [item for item in os.listdir(root_directory) 
...
def format_whatsapp_message(folder_pages, total_pages, total_elapsed):
    """
    Formata os resultados da contagem para envio via WhatsApp
    """
    message = "📊 *Relatório de Contagem de Páginas PDF*\n\n"
    ...
    message += f"⏱️ *Tempo de execução:* {total_elapsed:.2f} segundos"
...
def main():
    ...
    if folder_pages:
        message = format_whatsapp_message(folder_pages, total_folder_pages, total_elapsed)
