...
def extract_employee_name(folder_name):
    """
    Extrai o nome do funcionário do nome da pasta
    Ex: "1.LEIANE" -> "LEIANE"
    """
    # Remove números e pontos do início
    name = re.sub(r'^\d+\.?\s*', '', folder_name)
    # Remove barras e subpastas (para pastas VICTORIA)
    name = name.split('/')[0]
    return name.strip()
...
def load_previous_results():
    """
    Carrega os resultados da última execução
    """
...
def save_current_results(folder_pages_normal, folder_pages_victoria, total_pages_before_victoria):
    """
    Salva os resultados atuais para comparação futura
    """
    history_path = get_history_file_path()
    try:
        data = {
...
def calculate_changes(current_normal, current_victoria, previous_data):
    """
    Calcula as mudanças comparando com a última leitura
    Retorna: (changes_normal, changes_victoria)
    """
    changes_normal = {}
    changes_victoria = {}
        ...
        for folder, pages in sorted(folder_pages_normal.items()):
            ...
            if folder in changes_normal:
                ...
                if diff > 0:
                    ...
                    change_info = f" ({diff})"
            message += f"Pasta '{folder}': {pages} páginas{change_info}\n"
        message += "\n"
